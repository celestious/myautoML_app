...
st.set_page_config(page_title='This is a Machine Learning Algorithm Comparison App',
    layout='wide')
#---------------------------------#
# Model building
def build_model(df):
    ...
    st.markdown('**1.0. Dataset dimension**')
    st.write('X')
    st.info(X.shape)
    st.write('Y')
    st.info(Y.shape)

    st.markdown('**1.1. Details on the Variable**:')
    st.write('X variable (first 20 are shown below)')
    ...
    with st.markdown('**RMSE (capped at 60)**'):
        # Tall
...
In this implementation, the **lazypredict** library is used to build different machine learning models at once.
Developed by:Celestin Ntemngwa.  credits to Data professor
""")
...
if uploaded_file is not None:
    ...
    st.info('Awaiting for you to upload  CSV file.')
